...
def generate_image():
    ...
    if not prompt:
        ...
        if messages and len(messages) > 0:
            ...
            # Для моделей генерации изображений используем только последний запрос, а не всю историю
            # Очищаем историю, оставляя только последний запрос пользователя
            if "messages" in request_data and len(request_data["messages"]) > 1:
                system_messages = [msg for msg in request_data["messages"] if msg.get("role") == "system"]
                if last_message.get("role") == "user":
                    # Оставляем только системные сообщения и последнее сообщение пользователя
                    request_data["messages"] = system_messages + [last_message]
                    logger.debug(f"[{request_id}] Image generation: keeping only last user message")
    ...
    if model == "dall-e-3":
        ...
        if aspect_ratio == "1024x1024" or aspect_ratio == "1024×1024":
            aspect_ratio = "1:1"
        ...
        if ar_matches:
            ...
            # Если параметр --ar не найден в промпте, разбираем aspect_ratio из запроса
            if ":" in aspect_ratio:
                try:
                    ...
                    # При ошибке парсинга используем дефолтные значения
                    aspect_width = 1
                    aspect_height = 1
    ...
    elif model in ["black-forest-labs/flux-1.1-schnell", "flux-1.1-schnell"]:
        payload = {
            "type": "IMAGE_GENERATOR",
            "model": "black-forest-labs/flux-1.1-schnell",
            "promptObject": {
                "prompt": prompt,
                "num_outputs": request_data.get("n", 1),
                "aspect_ratio": request_data.get("aspect_ratio", "1:1"),
    ...
    elif model in ["leonardo-diffusion-xl", "lxl", "leonardo", "leonardo-ai"]:
        payload = {
            "type": "IMAGE_GENERATOR",
        ...
        for key in ["width", "height", "steps", "guidance_scale"]:
            if key in request_data:
                payload["promptObject"][key] = request_data[key]
                
        # Проверяем наличие параметра aspect_ratio
        if "aspect_ratio" in request_data:
            aspect_ratio = request_data["aspect_ratio"]
            if ":" in aspect_ratio:
                try:
                    w, h = aspect_ratio.split(":")
                    payload["promptObject"]["width"] = int(w) * 512
                    payload["promptObject"]["height"] = int(h) * 512
                except (ValueError, IndexError):
                    logger.error(f"[{request_id}] Invalid aspect_ratio format: {aspect_ratio}")
                    
    else:
        logger.error(f"[{request_id}] Unsupported model: {model}")
        return ERROR_HANDLER(1044, model)

    # Отправляем запрос через прокси
    url = "https://api.1min.ai/api/features/generator/imagine"
    logger.info(f"[{request_id}] Requesting image generation from {url}")
    logger.debug(f"[{request_id}] Payload: {json.dumps(payload)[:500]}...")

    try:
        # Отправляем запрос с заголовками API-KEY и ожидаем ответа
        response = requests.post(url, json=payload, headers=headers)
        response_text = response.text
        logger.debug(
            f"[{request_id}] Got response status: {response.status_code}"
        )
        ...
        if response.status_code == 200:
            try:
                ...
                logger.debug(
                    f"[{request_id}] Got JSON response: {json.dumps(response_json)[:500]}..."
                )

                # Проверяем наличие полей в ответе
                if "imageList" in response_json:
                    ...
                    # Добавляем URL изображений в response.data
                    for image_url in response_json["imageList"]:
                        openai_response["data"].append({"url": image_url})
                    
                    logger.info(f"[{request_id}] Successfully generated {len(openai_response['data'])} images with {model}")
                    return jsonify(openai_response)
                elif "status" in response_json and response_json["status"] == "error":
                    # Обрабатываем ошибки от API
                    error_message = response_json.get("message", "Unknown error")
                    logger.error(f"[{request_id}] API returned error: {error_message}")
                    return jsonify({
                        "error": {
                    ...
                    logger.error(f"[{request_id}] Unexpected response format: {response_json}")
                    return ERROR_HANDLER(1403)
            except json.JSONDecodeError:
                logger.error(
                    f"[{request_id}] Failed to parse JSON response: {response_text[:200]}..."
                )
                return ERROR_HANDLER(1402)
        elif response.status_code == 400:
            # Пытаемся извлечь сообщение об ошибке из ответа
            try:
                error_data = response.json()
                error_message = error_data.get("message", "Unknown error")
                logger.error(f"[{request_id}] API returned 400 error: {error_message}")
                ...
                logger.error(f"[{request_id}] API returned 400 error with invalid JSON")
                return ERROR_HANDLER(1402)
        elif response.status_code == 401:
            logger.error(f"[{request_id}] API returned 401 unauthorized")
            return ERROR_HANDLER(1401)
        elif response.status_code == 429:
            logger.error(f"[{request_id}] API returned 429 rate limit")
            return ERROR_HANDLER(1429)
        else:
            logger.error(
                f"[{request_id}] API returned unexpected status code: {response.status_code}"
            )
            return ERROR_HANDLER(1400 + response.status_code if response.status_code < 100 else 1500)

    except requests.RequestException as e:
        logger.error(f"[{request_id}] Request failed: {str(e)}")
        return ERROR_HANDLER(1500)
    except Exception as e:
...
def image_variations():
    ...
    # Создаем уникальный ID для запроса
    request_id = str(uuid.uuid4())
    logger.error(f"[{request_id}] Image variations endpoint is not fully implemented yet")
    return jsonify({"error": "Image variations functionality is not currently available"}), 501
