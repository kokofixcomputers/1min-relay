...
- **OpenClaw tool-calling**: реализован **в отдельном сервисе `openclaw-bridge`** (см. ниже). Сам `1min-relay` всегда **отбрасывает** `tools` / `tool_choice` / `parallel_tool_calls`, чтобы оставаться “тонким” прокси и не ломать streaming для обычных клиентов.
